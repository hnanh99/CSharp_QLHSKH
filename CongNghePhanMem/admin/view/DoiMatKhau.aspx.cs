...
namespace CongNghePhanMem.admin.view
{
    public partial class DoiMatKhau : System.Web.UI.Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            {
                DataTable CheckChangePassword = Model.Dbadmin.CheckChangePassword(Login.User_Id);
                if (CheckChangePassword != null)
                {
                    if (CheckChangePassword.Rows.Count > 0)
                    {
                        if (CheckChangePassword.Rows[0]["_Password"].ToString() == txtPW.Text)
                        {
                            ...
                            {
                                DataTable ChangePassword = Model.Dbadmin.ChangePassword(Login.User_Id, txtMKmoi.Text);
                                ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "script", "alert('Đổi mật khẩu thành công'); setTimeout(function(){ window.location.href = '/admin/view/Home.aspx'; }, 0);", true);
